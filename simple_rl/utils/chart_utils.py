...
def plot(results, experiment_dir, agents, plot_file_name="", conf_intervals=[], use_cost=False, cumulative=False, episodic=True, open_plot=True, track_disc_reward=False, figure_title=None):
    '''
    Args:
    ...
    for i, agent_name in enumerate(agents):
        ...
        y_axis = results[i]
        x_axis = list(drange(X_AXIS_START_VAL+1, X_AXIS_START_VAL + len(y_axis) * X_AXIS_INCREMENT + 1, X_AXIS_INCREMENT))
        ...
        marker_every = 3
        pyplot.plot(x_axis, y_axis, color=series_color, marker=series_marker, markersize=4, markevery=marker_every, label=agent_name)    
